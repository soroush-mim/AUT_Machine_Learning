...
def K_fold_sklearn(X , Y, K = 1 ,folds_num = 10):
    """this function uses k-fold cross validation and KNN with sklearn for predicting labes
    Args:
        X ([numpy array m*n]): [features]
        Y ([numpy array m*1]): [targets]
        K (int, optional): [num of neighbours for KNN]. Defaults to 1.
    ...
    """    
    #initializing predictions vector
    sk_Y_pred = np.zeros((Y.shape[0] , 1))
    #picking folds ranges
    folds = np.linspace(0.0, 1.0, num=folds_num+1)
    #number of samples
    m = X.shape[0]
    ...
    for i in range(folds_num):
        ...
        sk_Y_pred[start:end] = knn.predict(x_test).reshape(end - start,1)
    return sk_Y_pred
...
#calculating 10-fold cross validation with KNN for k = 1,3,5,7,15,30
for k in [1,3,5,7,15,30]:
    ...
    y_pred2 = K_fold_sklearn(X , Y , k)
    end2 = time.time()
    
    print('K = ' , k)
    print('time for our KNN: ' , end - start)
    print('time for sklearn KNN: ' , end2 - end)
    print('accuracy for our KNN: ' ,accuracy(Y , y_pred))
    print('accuracy for sklearn KNN: ' ,accuracy(Y , y_pred2))
    print('confusion_matrix for our KNN')
    confusion_matrix1(Y , y_pred)
    print('confusion_matrix for our sklearn KNN')
    confusion_matrix1(Y , y_pred2)
    print()