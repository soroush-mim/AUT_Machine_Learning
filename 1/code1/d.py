...
def plot_curve(x , y , theta , fig_name , d , alpha , train_error , test_error , iter_num , reg):

    """
    a function for generating plots and saving them
    """
    xx = np.linspace(-1.3, 1.3,100000)

    coeffs = list(map(float , theta))

    plt.plot(x[:,1] , y , '.') #plotting data
    plt.plot(xx, PolyCoefficients(xx, coeffs) , label = 'lambda= ' + str(reg)+' train error= ' + str(train_error) + ' \ntest error = ' + str(test_error)) #plotting the curve
...
for j in reg_nums:
    ...
    theta = GD(y_train , x_train , reg = j , d = d , iter_num=iter_num , alpha = alpha)

    test_error = MSE(scaling(make_featurs(x_test,d),d) ,y_test , theta , y_test.shape[0])

    train_error = MSE(scaling(make_featurs(x_train,d),d) ,y_train , theta , y_train.shape[0])

    plot_curve(scaling(make_featurs(x,d),d) , y , theta , str(j*10) , d , alpha , train_error , test_error , 2000 , j)
